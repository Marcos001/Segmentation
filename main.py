...
#calculando o tempo com kmeans em imagens 400 x 400 cinzas - roda 30 vzs
for i in range(3):
    tempo_k_means = timeit.timeit("kmeans_cv2({})".format("'"+lista_400_zinza[i]+"', '/home/nig/PycharmProjects/Segmentation/data/segmt/kmeans_400_cinza"+str(i)+".png'"), setup="from __main__ import kmeans_cv2", number=1)
    vari = 'execução '+str(i)+' com imagem '+lista_400_zinza[i]+' = '+str(tempo_k_means)
    arquivo_kmeans.write(vari+'\n')

#calculando o tempo com kmeans em imagens 400 x 400 rgb - roda 30 vzs
for i in range(3):
    tempo_k_means = timeit.timeit("kmeans_cv2({})".format("'"+lista_400_rgb[i]+"', '/home/nig/PycharmProjects/Segmentation/data/segmt/kmeans_400_rgb"+str(i)+".png'"), setup="from __main__ import kmeans_cv2", number=1)
    vari = 'execução '+str(i)+' com imagem '+lista_400_zinza[i]+' = '+str(tempo_k_means)
    arquivo_kmeans.write(vari+'\n')
...
for i in range(3):
    tempo_otsu = timeit.timeit(
        "binarizando_com_outsu({})".format("'"+lista_400_zinza[i]+"', '/home/nig/PycharmProjects/Segmentation/data/segmt/otsu"+str(i)+".png'"),setup="from __main__ import binarizando_com_outsu", number=1)
